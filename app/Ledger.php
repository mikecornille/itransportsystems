...
class Ledger extends Model
{
  ...
  //       ->whereRaw("STR_TO_DATE(`cleared_date`, '%m/%d/%Y') >= STR_TO_DATE('{$start_raw}', '%m/%d/%Y')")
    ...
    public function assetsITSQuery($start, $end)
    {
        $assetsITS = Ledger::select('date', 'type', 'type_description', 'journal_entry_number', 'account_name', 'memo', 'payment_method', 'payment_amount', 'deposit_amount')->whereBetween('date', [$start, $end])->where('type_description', 'Asset')->get();
