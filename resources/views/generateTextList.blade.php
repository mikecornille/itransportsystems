...
@foreach ($loads as $load)
    ...
    <p class="text-primary">Good Morning this is {{ \Auth::user()->name }} with International Transport Systems, I was checking to see how you are looking for the {{ $load->is_full_load == 'YES' ? 'FULL' : 'PARTIAL' }} LOAD with {{ $load->piece_count }} piece(s) picking up at {{ $load->pick_company }} in {{ $load->pick_city . ', ' . $load->pick_state }}?  PRO # {{ $load->id }}</p>
